    ...
    public void Open() => DalamudInterface.Instance.Framework.RunOnFrameworkThread(() => {
        ...
        if (InternalAddon is null) {
            ...
            if (InternalAddon is not null) {
                ...
                var nameBytes = System.Text.Encoding.UTF8.GetBytes(InternalName + "\0");
                fixed (byte* namePtr = nameBytes) {
                    AtkStage.Instance()->RaptureAtkUnitManager->InitializeAddon(&addonPtr, namePtr, 0, null);
                }
                InternalAddon = addonPtr;

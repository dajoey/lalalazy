...
internal partial class SGE
{
    ...
    private static bool UseRaidwide(ref uint actionID)
    {
        ...
        {
            // Shield FIRST: Eukrasia -> Eukrasian Prognosis. Mark the shield gate only once
            // the actual Prognosis goes out, so the Eukrasia -> Prognosis two-step is never
            // cut off mid-sequence and a mitigation can follow on the next GCD.
            if (HasStatusEffect(Buffs.Eukrasia))
            {
                AutoRotationController.MarkRaidwideShieldUsed();
                actionID = OriginalHook(Prognosis);
            }
            else actionID = Eukrasia;
            return true;
        }

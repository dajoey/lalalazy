...
internal sealed class MarketAutomation : Window, IDisposable
{
  ...
  private unsafe bool? RunLapDepositMover(string retainerName)
  {
    ...
    // 0.1.47.0: the deposit-only lap runs whether or not this board can list this sweep.
    // Depositing assigned stock into the assigned retainer's own pages is the satisfied state
  ...
  private bool? BuildListingStepsNow(List<Step> steps)
  {
    ...
    // 0.1.47.0: the mover organizes inventory whether or not this board can list this
    // session. A satisfied retainer holds every assigned item listed or in its own pages, and

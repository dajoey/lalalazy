...
public static class Communicator
{
  ...
  public static void PrintSweepDone(int listed, int failures)
  {
    if (!Plugin.Configuration.ShowAutoMarketMessages && listed == 0 && failures == 0)
      return;

    var text = listed == 0 && failures == 0
      ? "done."
      : $"done: {listed} new listing(s){(failures > 0 ? $", {failures} skipped (stock moved)" : string.Empty)}.";

...
public static class Communicator
{
  ...
  public static void PrintSweepDone(int listed, int failures, int vendored = 0, int heldBack = 0, int vendorFailures = 0, int pulled = 0, int unconfirmed = 0)
  {
    var didAnything = listed != 0 || failures != 0 || vendored != 0 || heldBack != 0 || vendorFailures != 0 || pulled != 0 || unconfirmed != 0;
    ...
    if (didAnything)
      Svc.Log.Information("[LMC] Auto-Market run " + FormatDoneLine(listed, failures, vendored, heldBack, vendorFailures, pulled, unconfirmed));

    if (!Plugin.Configuration.ShowAutoMarketMessages && !didAnything)
      return;

    Svc.Chat.Print(Prefix + FormatDoneLine(listed, failures, vendored, heldBack, vendorFailures, pulled, unconfirmed));
  }
  ...
  /// </summary>
  public static string FormatDoneLine(int listed, int failures, int vendored, int heldBack, int vendorFailures, int pulled = 0, int unconfirmed = 0)
    => AutoMarket.DoneLine.Format(listed, failures, vendored, heldBack, vendorFailures, pulled, unconfirmed);

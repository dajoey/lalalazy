...
### Notes
- **Shipped to production with 0.1.0.12 (2026-08-09).** In-game gate waived: every changed statement executes only when `DataCenter.IsPvP` is false, so in-PvP behavior is provably unchanged; the empirical check is that the PvE `no rotation found` warning stops firing (issue #3).

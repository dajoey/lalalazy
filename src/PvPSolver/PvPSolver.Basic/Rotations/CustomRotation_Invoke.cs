...
public partial class CustomRotation
{
	...
	private void UpdateHealingActions(JobRole role, out IAction? act)
	{
		...
		try
		{
			...
			else
			{
				...
				ActionHealAreaAbility = HealAreaAbility(RecuperatePvP, out act) ? act : null;
				ActionHealSingleAbility = HealSingleAbility(RecuperatePvP, out act) ? act : null;
			}
		}
		catch (Exception ex)
		{
	...
	private void UpdateDefenseActions(out IAction? act)
	{
		...
		try
		{
			ActionDefenseAreaAbility = DefenseAreaAbility(RecuperatePvP, out act) ? act : null;
			ActionDefenseSingleAbility = DefenseSingleAbility(RecuperatePvP, out act) ? act : null;
		}
	...
	private void UpdatePositionalActions(JobRole role, out IAction? act)
	{
		...
		ActionAntiKnockbackAbility = AntiKnockback(role, RecuperatePvP, out act) ? act : null;
	}

	private void UpdateMovementActions(out IAction? act)
	{
		IBaseAction.TargetOverride = TargetType.Move;
		bool movingTarget = MoveForwardAbility(RecuperatePvP, out act);
		IBaseAction.TargetOverride = null;
		ActionMoveForwardAbility = movingTarget ? act : null;

		ActionMoveBackAbility = MoveBackAbility(RecuperatePvP, out act) ? act : null;
		ActionSpeedAbility = SpeedAbility(RecuperatePvP, out act) ? act : null;
	}
	...
	private IAction? Invoke(out IAction? gcdAction)
	{
		...
		try
		{
			...
			else
			{
				...
				IBaseAction.IgnoreClipping = true;
				if (Ability(RecuperatePvP, out IAction? ability))
				{
					return ability;
				}
				IBaseAction.IgnoreClipping = false;

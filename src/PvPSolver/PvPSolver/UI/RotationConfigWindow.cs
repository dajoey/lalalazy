...
public partial class RotationConfigWindow : Window
{
	...
	private void DrawDiagnosticInfoCube()
	{
		...
		{
			_ = diagInfo.AppendLine($"PvP Solver v{typeof(RotationConfigWindow).Assembly.GetName().Version?.ToString() ?? "?.?.?"}");
		...
		else
		{
			...
			_ = diagInfo.AppendLine($"PvP Solver v{_cachedDiagInfo.RSRVersion}");
	...
	private static string GetDynamicHintText(int index)
	{
		...
		return "Thank you for using PvP Solver!";
	}
	...
	internal static void StatusPopUp(string popupId, Status[] allStatus, ref string searching, Action<Status> clicked, uint notLoadId = 0, float size = 32)
	{
		...
		if (popup)
		{
			...
			using var child = ImRaii.Child("PvP Solver Add Status", new Vector2(-1, ChildHeight * Scale));
			if (child)
			{
	...
	private static void DrawActionsList(string name, HashSet<uint> actions)
	{
		...
		ImGui.PushID(name);
		uint removeId = 0;
		string popupId = $"PvP Solver Action Popup{name}";
	...
	private static void DrawListTerritories()
	{
		...
		if (table)
		{
			...
			{
				ImGui.SetNextItemWidth(width);
				if (ImGui.InputTextWithHint($"##PvP Solver Territory Provoke Name {i}",
				...
				ImGui.SameLine();
				if (ImGuiEx.IconButton(FontAwesomeIcon.Ban, $"##PvP Solver Remove Territory Provoke Name {i}"))
				{
					removeIndex = i;
				}
			}

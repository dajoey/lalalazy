...
using LazyOccultCrescent.ItemHelpers;
...
public class CarrotHuntPanel : Panel
{
    ...
    public CarrotHuntPanel()
    {
        ...
        Svc.Log.Information(
            $"[CarrotHunt] territory {ZoneData.CurrentTerritory}: {carrotCount} carrot node(s), {aethernetCount} shard(s).");

        if (carrotCount == 0)
        {
            Svc.Log.Error(
                "[CarrotHunt] No carrot nodes for this zone. CarrotData is South Horn only; North Horn nodes " +
                "must be collected before this sweep can produce anything. Refusing to write an empty data file.");
        }

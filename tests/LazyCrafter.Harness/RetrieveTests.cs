...
internal static class RetrieveTests
{
    ...
    public static readonly List<(string Name, Func<bool> Check)> Tests = new()
    {
        ...
        ("a partial pull leaves a Retrieve for the remainder only, not the original quantity", () =>
        {
            ...
            var inv = new MovableInventory().SetElsewhere(World.Ingot, 2, "retainer RetainerC").Set(World.Leather, 1);
        ...
        ("The cart shape: 107 runs, every material on one retainer -> one Retrieve per material at 107x scale", () =>
        {
            ...
            var inv = new MovableInventory()
                .SetElsewhere(World.Ingot, 300, "retainer RetainerC")
                .SetElsewhere(World.Leather, 200, "retainer RetainerC");
            ...
            if (ingot.Places != "retainer RetainerC") return false;
        ...
        ("a one-item retrieve-only plan is not empty and is not craft work", () =>
        {
            ...
            var p = new DispatchPlan.Plan([], [], [], [], [], [], [], [new DispatchPlan.Retrieve(World.Ingot, 5, [new StoredElsewhere("retainer RetainerC", 5)])]);
            return !p.IsEmpty && !p.HasWork && p.Retrievals.Single().Detail == "5 on retainer RetainerC";
        }),
